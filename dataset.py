...
de_vocab.set_default_index(UNK_IDX) # 设置未知token返回的index
en_vocab=build_vocab_from_iterator(en_tokens,specials=[UNK_SYM, PAD_SYM, BOS_SYM, EOS_SYM],special_first=True) # 英语token词表
en_vocab.set_default_index(UNK_IDX) # 设置未知token返回的index

# 句子特征预处理
def de_preprocess(de_sentence):
    tokens = de_tokenizer(de_sentence) # 对德语句子进行分词处理
    tokens = [BOS_SYM] + tokens + [EOS_SYM] # 添加开始符 和 结束符
    ids = de_vocab(tokens)
    return tokens, ids
...
if __name__ == '__main__':
    # 打印德语 和 英语的词表大小
    print('de vocabs size:',len(de_vocab))
    print('en vocabs size:',len(en_vocab))
