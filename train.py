...
if __name__ == '__main__':
    ...
    dataloader=DataLoader(dataset,batch_size=32,shuffle=True,num_workers=4,persistent_workers=True,collate_fn=collate_fn)

    # 模型
    try:
        transformer=torch.load('checkpoints/model.pth')
    except:
        transformer=Transformer(enc_vocab_size=len(en_vocab),dec_vocab_size=len(de_vocab),emb_size=512,q_k_size=64,v_size=64,f_size=2048,head=8,nblocks=6,dropout=0.1,seq_max_len=SEQ_MAX_LEN).to(DEVICE)

    # 损失函数和优化器
    loss_fn=nn.CrossEntropyLoss(ignore_index=PAD_IDX) # 样本正确输出序列的PAD词不参与损失计算
    optimizer = torch.optim.SGD(transformer.parameters(), lr=1e-3, moment=0.99)
    ...
    for epoch in range(EPOCHS):
        ...
        for pad_enc_x, pad_dec_x in dataloader:
            ...
            loss.backward()
            optimizer.step()
        torch.save(transformer, 'checkpoints/model.pth')